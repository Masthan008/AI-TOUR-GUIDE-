...
export const ErrorDisplay: React.FC<ErrorDisplayProps> = ({ message, rawError, onRetry }) => {
    ...
    <div className="text-center p-8 bg-black/20 backdrop-blur-lg rounded-2xl shadow-2xl shadow-red-500/10 border border-red-500/50 max-w-md w-full animate-slide-up-fade-in">
      ...
      {rawError && (
        <div className="mt-4 text-left text-xs">
          <button onClick={() => setShowDetails(!showDetails)} className="text-gray-400 hover:text-white underline">
            {showDetails ? 'Hide Details' : 'Show Details'}
          </button>
          {showDetails && (
            <pre className="mt-2 p-2 bg-gray-900/50 rounded-md whitespace-pre-wrap break-all text-gray-300 font-mono">
              <code>{rawError}</code>
            </pre>
          )}
        </div>
